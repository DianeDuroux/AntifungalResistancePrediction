...
def merge_fungi_data(work_dir, albican_zip, non_albican_zip, maccs_file, smiles_file,
                       output_maccs_csv, output_smiles_csv):

    # Set working directory
    os.chdir(work_dir)

    # --- Unzip Parquet files ---
    with zipfile.ZipFile(albican_zip, 'r') as zip_ref:
        zip_ref.extractall()

    with zipfile.ZipFile(non_albican_zip, 'r') as zip_ref:
        zip_ref.extractall()

    # --- Load unzipped Parquet files ---
    albican_parquet = albican_zip.replace(".zip", "")
    non_albican_parquet = non_albican_zip.replace(".zip", "")

    data_albican = pd.read_parquet(albican_parquet)
    data_non_albican = pd.read_parquet(non_albican_parquet)

    # --- Load CSV files ---
    maccs = pd.read_csv(maccs_file)
    smiles = pd.read_csv(smiles_file)

    # --- Merge on 'drug' column ---
    data_albican_merged_maccs = pd.merge(data_albican, maccs, on="drug", how="left")
    data_albican_merged_smiles = pd.merge(data_albican, smiles, on="drug", how="left")

    data_non_albican_merged_maccs = pd.merge(data_non_albican, maccs, on="drug", how="left")
    data_non_albican_merged_smiles = pd.merge(data_non_albican, smiles, on="drug", how="left")

    # --- Concatenate ---
    data_all_maccs = pd.concat([data_albican_merged_maccs, data_non_albican_merged_maccs], ignore_index=True)
    data_all_smiles = pd.concat([data_albican_merged_smiles, data_non_albican_merged_smiles], ignore_index=True)

    # --- Save to CSV ---
    data_all_maccs.to_csv(output_maccs_csv, index=False)
    data_all_smiles.to_csv(output_smiles_csv, index=False)
...
def deploy_and_evaluate_model_external(
    ...
    extra_drop_cols = ['species_drug', 'stratify_group']
    drop_cols_train = [c for c in feature_removals + extra_drop_cols if c in train_set.columns]
    drop_cols_deploy = [c for c in feature_removals + extra_drop_cols if c in deployment_filtered.columns]

    X_train = train_set.drop(columns=drop_cols_train, errors='ignore')
    y_train = train_set['response']

    X_deploy = deployment_filtered.drop(columns=drop_cols_deploy, errors='ignore')
    y_deploy = deployment_filtered['response']
    species_deploy = deployment_filtered['species']
    drug_deploy = deployment_filtered['drug']
    ...
    if best_preproc_func == functions.mutual_information_feature_selection:
        X_train_pre, X_deploy_pre, _ = best_preproc_func(
            X_train, y_train_encoded, X_deploy,
            columns_prefix='X',
            **best_preproc_params
        )
    else:
    ...
    X_deploy_pre = X_deploy_pre.reindex(columns=X_train_pre.columns, fill_value=0)

    X_train_pre = (
        X_train_pre
        .replace([functions.np.inf, -functions.np.inf], functions.np.nan)
    )
    X_deploy_pre = (
        X_deploy_pre
        .replace([functions.np.inf, -functions.np.inf], functions.np.nan)
    ...
    sample_results.to_csv(
        f"{results_directory}/sample_specific_results_external.csv",
        index=False
    )
    print("Sample-specific results exported.")
    ...
    for species in species_deploy.unique():
        for drug in deployment_filtered.loc[
            deployment_filtered['species'] == species, 'drug'
        ].unique():

            mask = (species_deploy == species) & (drug_deploy == drug)

            y_true = y_deploy[mask]
            y_prob = y_pred_prob[mask]
            y_bin = y_pred[mask]
            ...
            has_both_classes = len(functions.np.unique(y_true)) == 2
            ...
            metrics.append({
                ...
                "auprc": average_precision_score(y_true, y_prob)
                if has_both_classes else functions.np.nan,
                ...
                if (tp + fn) > 0 else functions.np.nan,

                "specificity": tn / (tn + fp)
                if (tn + fp) > 0 else functions.np.nan,

                "accuracy": (tp + tn) / (tp + tn + fp + fn)
                if (tp + tn + fp + fn) > 0 else functions.np.nan,

                "very_major_error": fn / (fn + tp)
                if (fn + tp) > 0 else functions.np.nan,

                "major_error": fp / (tn + fp)
                if (tn + fp) > 0 else functions.np.nan,
    ...
    metrics_df.to_csv(
        f"{results_directory}/aggregated_metrics_external.csv",
        index=False
    )
    print("Aggregated metrics exported.")

    if metrics_df.empty:
        print("No metrics could be computed because no species-drug groups were found.")
        return {
    ...
    if filtered_metrics.empty:
        print(
            "No species-drug pairs had both outcome classes and enough samples "
            "for MCC plotting."
        )
    else:
        plot_df = filtered_metrics.sort_values(by='mcc', ascending=False).copy()
        plot_df['Species-Drug Pair'] = plot_df['species'] + ' - ' + plot_df['drug']
...
def generate_bootstrap_shap_uncertainty(
    data_path,
    deployment_path,
    output_dir,
    functions,
    ...
    """
    Estimate prediction and SHAP uncertainty by bootstrap-resampling the training set.

    Outputs:
    - sample_level_prediction_uncertainty.csv
    - sample_level_shap_uncertainty.csv
    ...
    # Filter selected species-drug pairs
    conditions = functions.pd.Series(False, index=deployment_set.index)
    for sp, dr in species_drug_filter:
        conditions |= ((deployment_set['species'] == sp) & (deployment_set['drug'] == dr))
    test_set = deployment_set[conditions].copy()

    if test_set.empty:
        raise ValueError("Filtered test set is empty. Check species_drug_filter.")

    # Fixed test set
    X_test_full = test_set.drop(columns=feature_removals)
    y_test = functions.LabelEncoder().fit_transform(test_set['response'])
    ...
    for b in range(n_bootstraps):
        ...
        train_idx = rng.choice(
            train_set.index,
            size=int(len(train_set) * sample_frac),
            replace=True
        )
        boot_train = train_set.loc[train_idx].copy()

        # Guard against degenerate resample
        if boot_train['response'].nunique() < 2:
            print("Skipped bootstrap: only one class present.")
            continue

        X_train_full = boot_train.drop(columns=feature_removals)
        y_train = functions.LabelEncoder().fit_transform(boot_train['response'])

        # Apply preprocessing
        if best_preproc_func == functions.mutual_information_feature_selection:
            X_train_pre, X_test_pre, selected = best_preproc_func(
                X_train_full, y_train, X_test_full, columns_prefix='X', **best_preproc_params
            )
            # In MI case, selected contains the chosen original spectral features
            top_features = selected
        else:
            X_train_pre, X_test_pre, loadings = best_preproc_func(
                X_train_full, X_test_full, **best_preproc_params
            )

            # If PCA was used, recover top original spectral features from loadings
            if loadings is not None:
                overall_importance = loadings.abs().sum(axis=1).sort_values(ascending=False)
                top_features = overall_importance.head(top_n_features).index.tolist()
            else:
                # No preprocessing case: use all spectral features
                top_features = [col for col in X_train_full.columns if col.startswith('X')]

        # Recombine selected spectral features + non-spectral (drug) features
        drug_columns = [col for col in X_train_full.columns if not col.startswith('X')]
        X_train_final = functions.pd.concat(
            [X_train_full[top_features], X_train_full[drug_columns]], axis=1
        )
        X_test_final = functions.pd.concat(
            [X_test_full[top_features], X_test_full[drug_columns]], axis=1
        )

        # Fit model
        model_instance = clone(best_model)
        pipeline = functions.Pipeline([
            ('scaler', functions.StandardScaler()),
        ...
        y_pred_prob = pipeline.predict_proba(X_test_final)[:, 1]
        y_pred_label = (y_pred_prob >= 0.5).astype(int)

        for i, row in test_set.reset_index(drop=True).iterrows():
        ...
        explainer = shap.KernelExplainer(pipeline_predict_proba, X_test_final)
        shap_values = explainer.shap_values(X_test_final)
        ...
        shap_df = functions.pd.DataFrame(shap_class1, columns=X_test_final.columns)
        ...
        for i, row in test_set.reset_index(drop=True).iterrows():
            for feature in shap_df.columns:
                shap_records.append({
                    ...
                    "drug": row["drug"],
                    "feature": feature,
                    "shap_value": shap_df.iloc[i][feature]
                })

    # -----------------------
    # Aggregate prediction uncertainty
    # -----------------------
    pred_df = functions.pd.DataFrame(prediction_records)
    pred_summary = pred_df.groupby(
        ["sample_id", "species", "drug", "true_label"]
    ).agg(
        mean_pred_prob=("pred_prob", "mean"),
        std_pred_prob=("pred_prob", "std"),
        q025_pred_prob=("pred_prob", lambda x: np.quantile(x, 0.025)),
        q975_pred_prob=("pred_prob", lambda x: np.quantile(x, 0.975)),
        positive_vote_rate=("pred_label", "mean")
    ).reset_index()

    # Entropy-like uncertainty
    eps = 1e-8
    pred_summary["prediction_entropy"] = -(
        pred_summary["mean_pred_prob"] * np.log(pred_summary["mean_pred_prob"] + eps) +
        (1 - pred_summary["mean_pred_prob"]) * np.log(1 - pred_summary["mean_pred_prob"] + eps)
    )

    pred_df.to_csv(os.path.join(output_dir, "sample_level_prediction_bootstraps.csv"), index=False)
    pred_summary.to_csv(os.path.join(output_dir, "sample_level_prediction_uncertainty.csv"), index=False)

    # -----------------------
    # Aggregate SHAP uncertainty
    # -----------------------
    shap_df_long = functions.pd.DataFrame(shap_records)

    sample_feature_summary = shap_df_long.groupby(
        ["sample_id", "species", "drug", "feature"]
    ).agg(
        mean_shap=("shap_value", "mean"),
        std_shap=("shap_value", "std"),
        mean_abs_shap=("shap_value", lambda x: np.mean(np.abs(x))),
        q025_shap=("shap_value", lambda x: np.quantile(x, 0.025)),
        q975_shap=("shap_value", lambda x: np.quantile(x, 0.975)),
        sign_consistency=("shap_value", lambda x: np.mean(np.sign(x) == np.sign(np.mean(x))))
    ).reset_index()

    # Pair-level feature summary
    feature_summary = sample_feature_summary.groupby(
        ["species", "drug", "feature"]
    ).agg(
        mean_of_mean_abs_shap=("mean_abs_shap", "mean"),
        mean_of_std_shap=("std_shap", "mean"),
        mean_sign_consistency=("sign_consistency", "mean")
    ).reset_index()

    # Rank features within pair
    feature_summary["feature_rank"] = feature_summary.groupby(
        ["species", "drug"]
    )["mean_of_mean_abs_shap"].rank(ascending=False, method="dense")

    shap_df_long.to_csv(os.path.join(output_dir, "sample_level_shap_bootstraps.csv"), index=False)
    sample_feature_summary.to_csv(os.path.join(output_dir, "sample_level_shap_uncertainty.csv"), index=False)
    feature_summary.to_csv(os.path.join(output_dir, "feature_level_summary.csv"), index=False)

    print("Bootstrap uncertainty analysis completed.")

def summarize_top_feature_stability(feature_summary, top_k=5):
    """
    Keep only the top-k features per species-drug pair.
    """
    top_features = feature_summary[feature_summary["feature_rank"] <= top_k].copy()
    return top_features.sort_values(
        ["species", "drug", "feature_rank", "mean_of_mean_abs_shap"],
...
def plot_prediction_uncertainty(csv_path):
    df = pd.read_csv(csv_path)

    df = df.sort_values("mean_pred_prob")

    lower_err = (df["mean_pred_prob"] - df["q025_pred_prob"]).clip(lower=0)
    upper_err = (df["q975_pred_prob"] - df["mean_pred_prob"]).clip(lower=0)
    ...
    plt.ylabel("Predicted probability resistant")
    plt.xlabel("Samples sorted by mean prediction")
    plt.title("Prediction uncertainty across samples")
    plt.tight_layout()
    plt.show()


def plot_feature_importance(
    csv_path, species, drug, top_k=10, ax=None, show_title=False,
    mz_start=2001, bin_size=3
):
    import pandas as pd
    import seaborn as sns
    import matplotlib.pyplot as plt
    ...
    df_pair = df[(df["species"] == species) & (df["drug"] == drug)].copy()
    df_pair = df_pair.sort_values("mean_of_mean_abs_shap", ascending=False).head(top_k)

    # Convert X feature index to m/z, same as plot_top_shap_features_on_spectra
    df_pair["feature_idx"] = (
        df_pair["feature"]
        .astype(str)
    ...
    df_pair["mz"] = (df_pair["feature_idx"] - 1) * bin_size + mz_start
    df_pair["feature_label"] = df_pair["mz"].astype(int).astype(str)
    ...
    sns.barplot(
        data=df_pair,
        y="feature_label",
        x="mean_of_mean_abs_shap",
        ax=ax
    )

    ax.errorbar(
        x=df_pair["mean_of_mean_abs_shap"],
        y=range(len(df_pair)),
        xerr=df_pair["mean_of_std_shap"],
        fmt="none",
        c="black"
    )
    ...
    if show_title:
        ax.set_title(f"{species} - {drug}")

def plot_sign_consistency(csv_path, species, drug, top_k=10):
    df = pd.read_csv(csv_path)

    df_pair = df[(df["species"] == species) & (df["drug"] == drug)]
    df_pair = df_pair.sort_values("mean_of_mean_abs_shap", ascending=False).head(top_k)
    ...
    sns.barplot(
        data=df_pair,
        y="feature",
        x="mean_sign_consistency"
    )

    plt.xlim(0,1)
    plt.xlabel("Sign consistency")
    plt.title(f"{species} - {drug}: SHAP stability")
    plt.show()

def export_driams_c(file_path, output_path):
    data = pd.read_csv(file_path)

    # Select only DRIAMS C
    driams_c = data[data['dataset'] == 'C'].copy()

    # (Optional) apply same cleaning steps EXCEPT removing C
    # e.g. intrinsic resistance removal if needed

    # Save in same format as deployment
    driams_c.to_csv(output_path, index=False)

    print(f"DRIAMS C exported to {output_path}")

def plot_top_shap_features_on_spectra(
    ...
    """
    Plot mean sensitive/resistant spectra and mark top SHAP-selected spectral features.
    This version does NOT restrict SHAP features to detected peaks.
    """
    ...
    feature_summary["species"] = feature_summary["species"].str.lower()
    ms["species"] = ms["species"].str.lower()

    def safe_name(x):
        return re.sub(r"[^A-Za-z0-9]+", "_", str(x)).strip("_")

    pairs = feature_summary[["species", "drug"]].drop_duplicates()
    ...
    for _, pair in pairs.iterrows():
        ...
        print(f"Processing: {species} - {drug}")

        fs_pair = feature_summary[
            (feature_summary["species"] == species) &
            (feature_summary["drug"] == drug)
        ].copy()

        fs_pair = fs_pair[fs_pair["feature"].astype(str).str.match(r"^X\d+$")]
        fs_pair = fs_pair.sort_values(
            "mean_of_mean_abs_shap",
            ascending=False
        ).head(top_n_features)

        if fs_pair.empty:
            print(f"No SHAP spectral features found for {species} - {drug}")
            continue
        ...
        fs_pair["mz"] = (fs_pair["feature_idx"] - 1) * bin_size + mz_start

        top_mz = fs_pair["mz"].drop_duplicates().head(top_n_markers).tolist()
        ...
        if subdata.empty:
            print(f"No MS data found for {species} - {drug}")
            continue

        x_cols = [
            col for col in subdata.columns
            if re.match(r"^X\d+$", str(col))
        ]

        sensitive = subdata[subdata["response"] == 0][x_cols]
        resistant = subdata[subdata["response"] == 1][x_cols]

        if sensitive.empty or resistant.empty:
            print(f"Skipping {species} - {drug}: missing sensitive or resistant samples")
            continue

        sensitive_means = sensitive.mean(axis=0, skipna=True).values
        resistant_means = resistant.mean(axis=0, skipna=True).values

        feature_indices = np.array([
            int(col.replace("X", "")) for col in x_cols
        ])

        mz_values = (feature_indices - 1) * bin_size + mz_start

        order = np.argsort(mz_values)
        mz_values = mz_values[order]
        sensitive_means = sensitive_means[order]
        resistant_means = resistant_means[order]

        # -------------------------
        # Plot: R-style appearance
        # -------------------------
        ...
        # Mark top SHAP features
        for mz in top_mz:
            idx = np.argmin(np.abs(mz_values - mz))
            y = max(sensitive_means[idx], resistant_means[idx])
        ...
        base_file = f"{safe_name(species)}_{safe_name(drug)}_top_shap_features"

        png_path = os.path.join(output_dir, f"{base_file}.png")
        pdf_path = os.path.join(output_dir, f"{base_file}.pdf")

        plt.savefig(png_path, dpi=600, bbox_inches="tight", facecolor="white")
        plt.savefig(pdf_path, bbox_inches="tight", facecolor="white")
        plt.close()

        fs_pair["species"] = species
        fs_pair["drug"] = drug
        all_top_features.append(fs_pair)

    if all_top_features:
        top_features_df = pd.concat(all_top_features, ignore_index=True)
        top_features_df.to_csv(
            os.path.join(output_dir, "top_shap_features_mz_no_peak_restriction.csv"),
            index=False
        )
